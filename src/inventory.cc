...
// 843803e: party equipment slots on the loot screen.
...
// 843803e: party equipment slot keys.
...
// 843803e: renamed from inventoryLootRightEquippedWeight; the party-slot
// feature keeps the stripped equipment so the loot screen can re-equip.
...
static void inventoryLootApplyLayout(int columns)
{
    ...
    // 843803e: party equipment slots sit to the left of the right avatar.
...
static void inventoryLootRenderPaneWeight(unsigned char* windowBuffer, int pitch, bool targetPane, Object* object, int extraWeight)
{
    // 8e60a2f/cb6a8ad: configurable weight/size indicator.
    ...
    int color = _colorTable[992]; // COLOR_GREEN
    ...
    if (objectTypeFromPid(object->pid) == OBJ_TYPE_CRITTER) {
        ...
        if (currentWeight > maxWeight) {
            color = _colorTable[31744]; // COLOR_RED
        }
...
// 843803e: party equipment management on the loot screen. The right pane
// party member's stripped equipment (CritterEquipped) is shown in two
// dedicated slots (weapon + armor) from which items can be equipped/
// unequipped directly.
...
static int buildPartyDisplayFid()
{
    ...
    return buildFid(objectTypeFromFid(partyBaseTarget->fid), partyBaseTarget->fid & 0xFFF, 0, weaponAnimationCode, ROTATION_NE);
}
...
static bool _setup_inventory(int inventoryWindowType)
{
    ...
    if (inventoryWindowType <= INVENTORY_WINDOW_TYPE_LOOT) {
        ...
        if (isNormalWindow) {
            ...
            if (backgroundFrmImage.lock(buildFid(OBJ_TYPE_INTERFACE, frmId, 0, 0, ROTATION_NE))) {
    ...
    // 843803e: reset party-slot state when (re)entering the window.
    partyTargetEquipped = nullptr;
    partyBaseTarget = nullptr;
...
static void _display_body(int fid, int inventoryWindowType)
{
    ...
    for (int index = 0; index < 2; index += 1) {
        int fid = fids[index];
        // 843803e: when party slots are active, the right pane displays the
        // party member's stripped-equipment FID (weapon animation included).
...
static void displayLootPanePartyName(unsigned char* windowBuffer, int windowPitch, const Rect& rect, int index)
{
    // 843803e: show the party member's name under the right avatar too
    // when the party slots are active.
    ...
    int oldFont = fontGetCurrent();
    fontSetCurrent(101);
    // cb6a8ad: tweaked label position for bigger critters like Marcus.
...
int inventoryOpenLooting(Object* looter, Object* target)
{
    ...
    // 843803e: keep the stripped equipment of the looted party member so the
    // party equipment slots can show/restore it.
    ...
    if (!_gIsSteal) {
        if (objectTypeFromFid(target->fid) == OBJ_TYPE_CRITTER) {
            ...
            if (critterCount > 1) {
                // 843803e: use the shared makeButton lambda (identical behavior).
    ...
    // 843803e: when quick-looting a party member with party_loot_and_barter,
    // Left/Right cycles the active right-side party target (with equipment
    // slots). Otherwise the previous left-pane switching behavior applies.
...
static InventoryMoveResult _move_inventory(Object* item, int slotIndex, Object* targetObj, bool isPlanting, int* stealXpOverridePtr)
{
    ...
    *stealXpOverridePtr = -1;
    ...
    if (isPlanting) {
        // 843803e: dropping onto the party slot equips the item directly.
                ...
                if (!skipMove && result != INVENTORY_MOVE_RESULT_CAUGHT_STEALING) {
                    if (!scriptHooks_InventoryMove(HOOK_INVENTORYMOVE_CONTAINER, item, targetObj)) {
                        ...
                        if ((item->flags & OBJECT_IN_RIGHT_HAND) != 0) {
                            targetObj->fid = buildFid(objectTypeFromFid(targetObj->fid), targetObj->fid & 0xFFF, animationTypeFromFid(targetObj->fid), 0, targetObj->rotation + 1);
                        }
